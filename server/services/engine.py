...
class Hey804Engine:
        ...
        if result["match"] is not None:
            ...
            if confidence >= 0.85:
                response = self._format_match(
                    match, related, channel, is_first_message, user_message=msg_stripped
                )
            else:
                ...
                if verified:
                    ...
                    if llm_result is not None:
                        ...
                        response = self._format_match(
                            llm_result, [], channel, is_first_message, user_message=msg_stripped
                        )
                    elif related:
    ...
    def _finalize(self, response: dict | str, channel: str, language: str) -> dict | str:
        ui_messages = {
            ...
            "see_steps": f"See all {len(response['action_steps'])} steps",
            "also_see": "Also see",
            "might_help": "These might help",
            "back_button": "Ask me something else",
            # Not really needed/positioned to be used
        ...
        if isinstance(response, str):
            ...
            print("action steps:", response["action_steps"])
